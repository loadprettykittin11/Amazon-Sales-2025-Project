...
WITH category_cancelled AS (
	SELECT 
		category,
		COUNT(*) AS order_cancelled,
		SUM(COUNT(*)) OVER() AS total_cancelled
	FROM amazon_sales
...
SELECT 
	category,
	order_cancelled,
	ROUND((order_cancelled / total_cancelled) * 100, 2) AS cancelled_rate
FROM category_cancelled
